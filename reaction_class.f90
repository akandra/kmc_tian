...
!              end if
      ...
      case(association_id)
        ...
        call lat%neighbor(ads,m_nn,row,col)
        ...
        if (ads_r2 < this%n_ads_total) then
          ...
          ! is not valid Fortran if rates is an array of derived types, 
          ! because Fortran does not support whole-row assignment for arrays of derived types 
          ! with assumed-size or allocatable components.          
          ! Thanks to GPT-4.1 for this explanation.
          do i=1,lat%n_max_nn+1
            this%hopping%rates(ads_r2,i) = this%hopping%rates(this%n_ads_total,i)
          end do
