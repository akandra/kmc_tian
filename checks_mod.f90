...
subroutine code_checks(lat, c_pars)
  ...
  call check_neighbor_lists(lat, c_pars)
...
subroutine check_neighbor_lists(lat, c_pars)
  ...
  write(6,*)
  write(6,'(A)') 'Humble report of neighbor lists consistency check:'
  if (all_is_okay) then
    write(6,'(A)') '  All neighbor lists are consistent.'
  else
    write(6,'(A)') '  Some errors found in neighbor lists!'
  end if
